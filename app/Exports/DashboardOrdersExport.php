...
class DashboardOrdersExport implements FromArray
{
    public function array(): array
    {
        ...
        foreach ($orderItems as $index => $item) {
            ...
            $rows[] = [
                $index + 1,
                $item->order->customer_name ?? $item->order->user->name ?? 'Không rõ',
                $item->order->customer_email ?? $item->order->user->email ?? 'Không rõ',
